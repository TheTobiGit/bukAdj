<template>
  ...
  <section
    class="h-svh w-screen bg-[#206A5D] flex flex-col text-[#EBECF1] p-4 gap-5 lg:hidden"
  >
    ...
    <form @submit.prevent="login" class="flex flex-col gap-5">
      <div class="flex flex-col gap-2">
        ...
        <input
          ...
          class="w-full p-2 text-black bg-[#EBECF1] focus:outline-none"
          required
        />
      </div>
      <div class="flex flex-col gap-2">
      ...
      <button
        type="submit"
        class="bg-[#EBECF1] text-[#1A1B25] p-2 rounded-lg text-center h-10"
      >
...
<script setup>
const { isLoading, signIn, email, password, googleAuth, isAuthenticating } = useAuth();
